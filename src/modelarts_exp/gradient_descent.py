...
This module contains the complete implementation of the gradient descent algorithm
for linear regression problems, including demonstration code.
"""
...
def run_demo():
    ...
    # 绘制拟合曲线
    plt.figure(figsize=(10, 5))
    plt.scatter(X[:, 1], y, label='True Values', color='blue', alpha=0.6)
    plt.plot(X[:, 1], y_pred, color='red', label='Predictions', linewidth=2)
    plt.xlabel('X')
    plt.ylabel('y')
